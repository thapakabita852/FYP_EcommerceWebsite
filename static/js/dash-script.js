window.addEventListener("DOMContentLoaded", () => {
  ...
  let observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      ...
      if (mutation.target.classList.contains("item-container")) {
        mutation.addedNodes[0].children[3].addEventListener(
          "click",
          addCartElm
        );
      }
      if (mutation.target.classList.contains("tbody")) {
        mutation.addedNodes[0].lastElementChild.addEventListener(
          "click",
          rmCartElm
        );
  ...
  function updateTotal() {
    ...
    rows.forEach((row) => {
      ...
      const elm = ary.find(
        (x) => x.firstElementChild instanceof HTMLInputElement
      );
